 ...
 * 微信公众平台C#版SDK
 * www.qq8384.com 版权所有
 * 有任何疑问，请到官方网站:www.qq8384.com查看帮助文档
...
namespace Weixin.Mp.Sdk
{
    ...
    public class MessageHandler
    {
        ...
        public static bool CheckSignature(string token)
        {
            ...
            //Logger.WriteTxtLog((tmpStr == signature).ToString());
            //Logger.WriteTxtLog((tmpStr + "=====" + signature).ToString());
            return true;//tmpStr == signature; //开发者获得加密后的字符串可与signature对比
        ...
        {
            //Logger.WriteTxtLog("fdsafsdfasd:" + token);
            //if (!CheckSignature(token))
            //{
            //    return null;
            //}

            //Logger.WriteTxtLog("1:" + token);

            string msgBody = string.Empty;
            Stream s = System.Web.HttpContext.Current.Request.InputStream;

            //Logger.WriteTxtLog("2:" + s);
            byte[] b = new byte[s.Length];
            s.Read(b, 0, (int)s.Length);
            msgBody = Encoding.UTF8.GetString(b);

            Logger.WriteTxtLog("3:" + msgBody);
            ...
            try
            {
                ...
                switch (strMsgType)
                {
                    ...
                    case "event":
                        ...
                        Logger.WriteTxtLog("k");
                        if (eventNode != null)
                        {
                            switch (eventNode.InnerText)
                            {
                                case "subscribe":
                                    Logger.WriteTxtLog("l");
                                    eventType = EventType.subscribe;
                                    Logger.WriteTxtLog("m");
                                    tmpNode = rootElement.SelectSingleNode("EventKey");
                                    Logger.WriteTxtLog("n");

                                    //try {
                                    //    Logger.WriteTxtLog(rootElement.SelectSingleNode("EventKey").ToString());
                                    //}
                                    //catch(Exception ex1) {
                                    //    Logger.WriteTxtLog(ex1.Message);
                                    //}

                                    //if (tmpNode != null)  // && (!string.IsNullOrWhiteSpace(tmpNode.InnerText)))  //&& tmpNode.InnerText.StartsWith("qrscene_")
                                    //{
                                    //    //扫描二维码关注事件
                                    //    Logger.WriteTxtLog("saomiao");
                                    ...
                                    //    };
                                    //    Logger.WriteTxtLog("saomiaoend");
                                    //    return scanSubEvt;
                                    //}
                                    //else
                                    //{
                                        //普通关注事件
                                    Logger.WriteTxtLog("o");
                                    Logger.WriteTxtLog("mesbody:" + msg.MessageBody);
        ...
        public static void TransferToAutoCustomer(string fromUserName, string toUserName) {
            ...
            TransferAutoCustomer msg = new TransferAutoCustomer()
            {
                ...
                FromUserName = fromUserName,
                ToUserName = toUserName,
                MsgType = MsgType.transfer_customer_service
            };
            Logger.WriteTxtLog(msg.ToXmlString());
            System.Web.HttpContext.Current.Response.Write(msg.ToXmlString());
        }
        ...
        public static bool SendImageReplyMessage(string accessToken, string fromUserName, string toUserName, string imgPath)
        {
            ...
            //Logger.WriteTxtLog("66666:");
            UploadMediaResponse response = mpClient.Execute(request);
            //Logger.WriteTxtLog("77777:");
            if (response.IsError)
            {
                Logger.WriteTxtLog(response.ErrInfo.ToString());
                //Logger.WriteTxtLog("88888:");
                return false;
            }
            else
            {
                Logger.WriteTxtLog("Send");
