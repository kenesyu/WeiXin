                ...
                if (!IsPostBack)
                {
                    if (Request.QueryString["id"]!= null && Request.QueryString["id"].ToString() != "")
                    {
                        ...
                        else
                        {
                            ...
                            if (dbHelper.ExecuteDataTable("select * from T_HGTQD_TP where openid = '" + openId + "'").Rows.Count == 0)
                            {
                                dbHelper.ExecuteNonQuery("insert into T_HGTQD_TP (openid,ToID) values ('" + openId + "'," + Convert.ToInt32(Request.QueryString["id"].ToString()) + ")");
                                dbHelper.ExecuteNonQuery("update T_HGTQD set CountNum = (CountNum + 1) where id = '" + Convert.ToInt32(Request.QueryString["id"].ToString()) + "'");
