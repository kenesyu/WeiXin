...
namespace WebAPP.WebApp.WXPay
{
    public partial class ConfirmPay : System.Web.UI.Page
    {
        ...
        private void BindData()
        {
            ...
            this.lblProductName.Text = this.Request.QueryString["Body"];
            this.UserOpenId = this.Request.QueryString["UserOpenId"];

            DataBaseHelper dbHelper = new DataBaseHelper(ConfigurationManager.ConnectionStrings["DB"].ToString());
            DataTable dt = dbHelper.ExecuteDataTable("select * from T_Products where productid = " + this.Request.QueryString["Attach"].ToString() + " and isopen = 1");
            if (dt.Rows.Count > 0)
            {
            ...
            this.lblOrderNo.Text = this.Request.QueryString["OrderSN"];
